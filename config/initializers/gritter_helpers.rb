Gritter::Helpers.module_eval do
  def add_gritter text, *args
    options = args.extract_options!
    options[:title] = "Notification" if options[:title].blank?
    options[:image] = asset_path("#{options[:image]}#{options[:image].to_s == 'progress' ? '.gif' : '.png'}") if %w(success warning error notice progress alert).include?(options[:image].to_s)
